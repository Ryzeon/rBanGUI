package me.ryzeon.bangui.utils.gui;
...
public class ButtonListener implements Listener {
		...
		if (openMenu != null) {
			...
			if (openMenu.getButtons().containsKey(event.getSlot())) {
				...
				if (event.isCancelled()) {
					Bukkit.getScheduler().runTaskLater(rBanGUI.getInstance(), () -> player.updateInventory(), 1L);
				}
			} else {
	...
	public void onInventoryClose(InventoryCloseEvent event) {
		...
		player.setMetadata("scanglitch", new FixedMetadataValue(rBanGUI.getInstance(), true));
	}
	...
	public void onPlayerMove(PlayerMoveEvent event) {
		...
		if (player.hasMetadata("scanglitch")) {
			player.removeMetadata("scanglitch", rBanGUI.getInstance());
